...
function build_fontconfig {
	...
	then
		...
		then
			...
			# TODO: -lfontconfig $XML2_LIBS $FREETYPE_LIBS
			#export FONTCONFIG_LIBS="-L/usr/local/lib -lfontconfig -lexpat -lfreetype"
			## TODO: -lfontconfig $(pkg-config --libs expat) $(pkg-config --libs freetype)
		else
...
function build_x264 {
	...
	then
		...
		./configure $CONFIGURE_ALL_FLAGS --enable-win32thread --bit-depth=10 --enable-strip --disable-cli
		make
		make install
	fi
}
